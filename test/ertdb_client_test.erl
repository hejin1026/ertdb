...
run() ->
	...
	C = c(),
	config_test(C, Key, [{compress, 1}, {dev, 8}, {maxtime, 60}, {mintime, 10}]),
	timer:sleep(20),
	io:format("rtk config:~p", [ertdb:lookup(Key)]),
	insert_test(C, Key, extbif:timestamp(), 123).
	% fetch_test(C, Key).
...
%% command
	...
	% ertdb_client_test:config_test(C, Key, [{compress, 1}, {dev, 8}, {his_dev, 16}, {maxtime, 60}, {mintime, 10}]).
	% ertdb:lookup(Key).
	
	% ertdb_client_test:insert_test(C, Key, extbif:timestamp(), 123).
	% ertdb_client_test:fetch_test(C, Key).	
	% ertdb_store_current:read(ertdb_store_current, Key).
			
	% ertdb_client_test:fetch_test(C, Key, extbif:timestamp({{2014,1,6}, time()}), extbif:timestamp()).	
	% ertdb_store_history:read(ertdb_store_history, Key, extbif:timestamp({{2014,1,6}, time()}), extbif:timestamp()).
